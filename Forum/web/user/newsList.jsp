...
<head>
    ...
    <script>
        function toggleLike(newsId) {
            var imgElement = document.getElementById('likeImg' + newsId);
            var countElement = document.getElementById('likeCount' + newsId);
            var currentCount = parseInt(countElement.innerHTML);
            var hasLiked = imgElement.getAttribute('data-liked') === 'true'; // 使用属性追踪是否点赞

            if (hasLiked) {
                // 取消点赞
                imgElement.src = '../images/zan.png';
                imgElement.setAttribute('data-liked', 'false'); // 更新点赞状态
                currentCount = Math.max(0, currentCount - 1); // 防止点赞数小于0
                countElement.innerHTML = currentCount;
            } else {
                // 点赞
                imgElement.src = '../images/zan_1.png';
                imgElement.setAttribute('data-liked', 'true'); // 更新点赞状态
...
<%--                    利用帖子id的唯一性，动态设置这个div 的id--%>
                    ...
                    <div style="width: 100%">
                        ...
                        <span style="margin-right: 10%;text-align: center">
                            <img id="likeImg${news.newsId}" src="${news.priseNum > 0 ? '../images/zan_1.png' : '../images/zan.png'}" width="32px" align="left"
                                 onclick="toggleLike('${news.newsId}', ${news.priseNum > 0});">
                            <span id="likeCount${news.newsId}">${news.priseNum}</span>
                        </span>
                        <span style="margin-right: 10%;text-align: center">
                            <img src="../images/comment.png" width="32px">
                        </span>
                    </div>
